...
class User{
  ...
  public function register($email, $screenName, $password) {
    ...
    $stmt->bindParam(":email", $email, PDO::PARAM_STR);
    $stmt->bindParam(":password", password_hash($password, PASSWORD_BCRYPT, ["cost" => 12]), PDO::PARAM_STR);
  ...
  public function checkPassword($password) {
    ...
    $count = $stmt->rowCount();
    if($count > 0 && password_verify($password, $dbPass)) {
      return true;
    } else {
      return false;
    }
    ...
    if(in_array($ext, $allowed_ext) === true) {
      if($error === 0) {
        if($fileSize <= 209272152) {
          ...
          move_uploaded_file($filetmp, $_SERVER['DOCUMENT_ROOT'].'/social_network/'.$fileRoot);
          return $fileRoot;
        } else {
